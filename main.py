# ────────────────────────────────────────────────────────────────────────────
#  BizPartner-AI   ·   FastAPI + OpenAI Assistants   ·   без внешнего логгера
# ────────────────────────────────────────────────────────────────────────────
...
# ── In-memory соответствие  lead_id ↔ thread_id ─────────────────────────
    ...
    try:
        ...
        while True:
            run = client.beta.threads.runs.retrieve(
                run_id=run.id,             # важно: keyword-аргументы
                thread_id=thread_id
            )
        ...
        traceback.print_exc(file=sys.stderr)   # чтобы видеть 500-ошибку в логах
